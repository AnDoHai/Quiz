(function ($$1) {
    ...
    var FilterMenu = function () {
        ...
        FilterMenu.prototype.dropdownFilterItem = function (td, self) {
            ...
            span.innerHTML = '';
            input.type = 'checkbox';
            input.value = $.isNumeric(value) && !$(td).hasClass("no-format") ? formatNumber(value.trim().replace(/ +(?= )/g, ''), $(td)) : value.trim().replace(/ +(?= )/g, '');
            ...
            dropdownFilterItem.innerHTML = dropdownFilterItem.innerHTML.trim() + ' ' + ($.isNumeric(value) && !$(td).hasClass("no-format") ? formatNumber(value, $(td)) : value);
            ...
            var selectedLists = filterMenus.map(function (filterMenu) {
                return {
                    ...
                    selected: filterMenu.inputs.filter(function (input) {
                        ...
                        var selectedValue = input.value.trim().replace(/ +(?= )/g, '');
    ...
    $$1.fn.excelTableFilter = function (options) {
        ...
        if (typeof options.captions === 'undefined') options.captions = {
            a_to_z: 'Sắp xếp A to Z',
            z_to_a: 'Sắp xếp Z to A',
            search: 'Tìm kiếm',
