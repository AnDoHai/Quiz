...
var userTestQuestionAnswerModule = {
    init: function () {
        userTestQuestionAnswerModule.openConfirmDelete();
        userTestQuestionAnswerModule.onClickActive();
        userTestQuestionAnswerModule.onClickSearch();
        userTestQuestionAnswerModule.onClickSorting();
        userTestQuestionAnswerModule.initCheckuserTestQuestionAnswer();
		userTestQuestionAnswerModule.openPopup();
    },
    initCheckuserTestQuestionAnswer: function () {
        var checkAll = $(".checkedAll");
        checkAll.each(function (index, element) {
            var rootParent = element.id;

            var isChecked = $(".userTestQuestionAnswer-item").find("input[data-root='" + rootParent + "']").val();
    ...
    openConfirmDelete: function () {
        $(document).on("click", ".icon-remove4", function () {
            ...
            var root = $("#userTestQuestionAnswer_modal_info").find(".confirm-yes");
            $(root).attr("onclick", userTestQuestionAnswerModule.executeDelete);
            $(root).attr("data-id", dataId);

            common.modal.show("userTestQuestionAnswer_modal_info");
        });
    },
    
    executeDelete: function () {
        $(document).on("click", "#btn_userTestQuestionAnswer", function () {
            var dataId = $(this).attr("data-id");
            $.ajax({
                url: "/UserTestQuestionAnswer/Delete",
                type: "POST",
                data: { UserTestQuestionAnswerId: dataId },
    ...
    onClickPaging: function (currentPage, sortColumn, sortDirection) {
        ...
        $.ajax({
            url: "/UserTestQuestionAnswer/Search",
            data: {
            ...
            success: function (response) {
                if (response.IsError === false && (undefined != response.HTML && "" !== response.HTML)) {
                    $("#main-userTestQuestionAnswer").html("");
                    $("#main-userTestQuestionAnswer").html(response.HTML);
                }
            }
        });
    },
    onClickSorting: function () {
        $(document).on("click", ".sorting,.sorting_asc,.sorting_desc", function () {
            ...
            userTestQuestionAnswerModule.onClickPaging(1, sortColumn, sortDirectionValue);
        });
    },
    onClickActive: function () {
        $(document).on("click", ".active", function () {
            ...
            $.ajax({
                url: "/UserTestQuestionAnswer/Invisibe",
                type: "POST",
                data: { UserTestQuestionAnswerId: dataId },
                success: function (data) {
                    if (data.IsError === false) {
    ...
    onClickSearch: function () {
        $(document).on("click", "#btnSearch", function () {
            userTestQuestionAnswerModule.onClickPaging(1);
        });
    }
}
