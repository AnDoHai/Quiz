...
var userTestQuestionModule = {
    init: function () {
        userTestQuestionModule.openConfirmDelete();
        userTestQuestionModule.onClickActive();
        userTestQuestionModule.onClickSearch();
        userTestQuestionModule.onClickSorting();
        userTestQuestionModule.initCheckuserTestQuestion();
		userTestQuestionModule.openPopup();
    },
    initCheckuserTestQuestion: function () {
        var checkAll = $(".checkedAll");
        checkAll.each(function (index, element) {
            var rootParent = element.id;

            var isChecked = $(".userTestQuestion-item").find("input[data-root='" + rootParent + "']").val();
    ...
    openConfirmDelete: function () {
        $(document).on("click", ".icon-remove4", function () {
            ...
            var root = $("#userTestQuestion_modal_info").find(".confirm-yes");
            $(root).attr("onclick", userTestQuestionModule.executeDelete);
            $(root).attr("data-id", dataId);

            common.modal.show("userTestQuestion_modal_info");
        });
    },
    
    executeDelete: function () {
        $(document).on("click", "#btn_userTestQuestion", function () {
            var dataId = $(this).attr("data-id");
            $.ajax({
                url: "/UserTestQuestion/Delete",
                type: "POST",
                data: { UserTestQuestionId: dataId },
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.IsError === false && (undefined != response.HTML && "" !== response.HTML)) {
                    $("#main-userTestQuestion").html("");
                    $("#main-userTestQuestion").html(response.HTML);
                }
            }
        });
    },
    onClickSorting: function () {
        $(document).on("click", ".sorting,.sorting_asc,.sorting_desc", function () {
            ...
            userTestQuestionModule.onClickPaging(1, sortColumn, sortDirectionValue);
        });
    },
    ...
    onClickSearch: function () {
        $(document).on("click", "#btnSearch", function () {
            userTestQuestionModule.onClickPaging(1);
        });
    }
}
