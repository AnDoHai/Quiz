...
function createTree(p_div,p_backColor,p_contextMenu) {
	var tree = {
		...
		createNode: function(p_text,p_expanded, p_icon, p_parentNode,p_tag,p_contextmenu, p_dataid, p_danhsachchitietid) {
			v_tree = this;
			node = {
                id: 'node_' + this.nodeCounter,
                dataid: p_dataid,
                danhsachchitietid: p_danhsachchitietid,
				text: p_text,
				icon: p_icon,
				parent: p_parentNode,
				expanded : p_expanded,
                ...
                elementLi: null,
                ///// Update DinhMuc
                updateDinhMuc: function () { v_tree.updateDinhMuc(this); },
                themMoiChiTiet: function () { v_tree.themMoiChiTiet(this); },
                deleteDinhMuc: function () { v_tree.deleteDinhMuc(this); },
                deleteDinhMucChiTiet: function () { v_tree.deleteDinhMucChiTiet(this); },
                changeStatus: function () { v_tree.changeStatus(this); },
                updateVatTu: function () { v_tree.updateVatTu(this); },
                ...
                createChildNode: function (p_text, p_expanded, p_icon, p_tag, p_contextmenu, p_dataid, p_danhsachchitietid) { return v_tree.createNode(p_text, p_expanded, p_icon, this, p_tag, p_contextmenu, p_dataid, p_danhsachchitietid); }
			}
				...
				if (p_parentNode==undefined) {
					...
					if (p_parentNode.childNodes.length==0) {
						...
						v_img.style.visibility = "visible";
                            v_img.src = '/Assets/lib/aimaraTree/images/collapse.png';
						v_img.id = 'toggle_off';
						}
						else {
							...
							v_img = p_parentNode.elementLi.getElementsByTagName("img")[0];
							v_img.style.visibility = "visible";
                            v_img.src = '/Assets/lib/aimaraTree/images/expand.png';
							v_img.id = 'toggle_on';
						}
					}
      ...
      this.adjustLines(document.getElementById(this.name),true);
		...
		drawNode: function(p_ulElement,p_node) {
			...
			if (p_node.childNodes.length == 0) {
                v_exp_col = createImgElement('toggle_off', 'exp_col','/Assets/lib/aimaraTree/images/collapse.png');
				...
				if (p_node.expanded) {
                    ...
                    v_exp_col = createImgElement('toggle_on', 'exp_col','/Assets/lib/aimaraTree/images/expand.png');
				}
			}
		...
		expandNode: function(p_node) {
			if (p_node.childNodes.length>0 && p_node.expanded==false) {
				...
				img.id="toggle_off";
                img.src = '/Assets/lib/aimaraTree/images/collapse.png';
		...
		collapseNode: function(p_node) {
			if (p_node.childNodes.length>0 && p_node.expanded==true) {
				...
				img.id="toggle_on";
                img.src = '/Assets/lib/aimaraTree/images/expand.png';
				elem_ul = img.parentElement.getElementsByTagName("ul")[0];
        ...
        ///// updateDinhMuc
        // p_node: Reference to the node;
        updateDinhMuc: function (p_node) {
            var url = "/DinhMucSanXuat/Create";
            var title = "Thêm mới định mức sản xuất";
            if (p_node != null && p_node.dataid != null) {
                url = "/DinhMucSanXuat/Edit/" + p_node.dataid;
                title = "Chỉnh sửa định mức sản xuất";
            }
            else if (p_node.danhsachchitietid != null) {
                var url = "/DinhMucSanXuat/Create?danhsachchitietid=" + p_node.danhsachchitietid;
            }
            else {
                return false;
            }
        ...
        ///// delete DinhMuc
        // p_node: Reference to the node;
        deleteDinhMuc: function (p_node) {
            if (p_node == null || p_node.dataid == null) {
                return false;
            }
            var dataId = p_node.dataid;
            var root = $("#dinhMucSanXuat_modal_info").find(".confirm-yes");
            $(root).attr("onclick", dinhMucSanXuatModule.executeDelete);
            $(root).attr("data-id", dataId);

            common.modal.show("dinhMucSanXuat_modal_info");
        },
        ///// delete DinhMuc
        // p_node: Reference to the node;
        changeStatus: function (p_node) {
            if (p_node == null || p_node.dataid == null) {
                return false;
            }
            var dataId = p_node.dataid;
            $.ajax({
                url: "/DinhMucSanXuat/Invisibe",
                type: "POST",
                data: { DinhMucSanXuatId: dataId },
                success: function (data) {
                    if (data.IsError === false) {
                        var nodeText = p_node.text;
                        if (nodeText.indexOf("Ẩn") >= 0) {
                            p_node.setText(nodeText.replace("Ẩn", "Hiện"));
                        }
                        else {
                            p_node.setText(nodeText.replace("Hiện", "Ẩn"));
                        }
        ...
        ///// themMoi ChiTiet
        // p_node: Reference to the node;
        themMoiChiTiet: function (p_node) {
            if (p_node == null || p_node.dataid == null) {
                return false;
            }
            var url = "/DanhSachChiTiet/Create" + "?dinhMucSanXuatId=" + p_node.dataid;
            var title = "Thêm mới danh sách chi tiết";
        ...
        ///// update VatTu
        // p_node: Reference to the node;
        updateVatTu: function (p_node) {
            if (p_node == null || p_node.parent == null || p_node.parent.dataid == null) {
                return false;
            }
            var url = "/DanhSachChiTiet/Create" + "?dinhMucSanXuatId=" + p_node.parent.dataid;
            var title = "Thêm mới danh sách chi tiết";
            if (p_node != null && p_node.danhsachchitietid != null) {
                url = "/DanhSachChiTiet/Edit/" + p_node.danhsachchitietid + "?dinhMucSanXuatId=" + p_node.parent.dataid;
                title = "Chỉnh sửa danh sách chi tiết";
            }
        ...
        ///// delete DinhMuc
        // p_node: Reference to the node;
        deleteDinhMucChiTiet: function (p_node) {
            if (p_node == null || p_node.danhsachchitietid == null) {
                return false;
            }
            var dataId = p_node.danhsachchitietid;
            var root = $("#dinhMucSanXuat_modal_info").find(".confirm-yes");
            $(root).attr("onclick", tree.executeDeleteDetail);
            $(root).attr("data-id", dataId);

            common.modal.show("dinhMucSanXuat_modal_info");
        },
        executeDeleteDetail: function () {
            $(document).on("click", "#btn_dinhMucSanXuat", function (event) {
                event.stopImmediatePropagation();
                var dataId = $(this).attr("data-id");
                $.ajax({
                    url: "/DanhSachChiTiet/Delete",
                    type: "POST",
                    data: { DanhSachChiTietId: dataId },
                    success: function (data) {
                        if (data.IsError === false) {
                            location.reload();
                        } else {
				...
				if (p_node.contextMenu!=undefined) {
					...
					for (var i=0; i<v_menu.elements.length; i++) (function(i){
						...
						if (v_menu.elements[i].submenu!=undefined) {
                            ...
                            v_span_more.appendChild(createImgElement(null, 'menu_img','/Assets/lib/aimaraTree/images/right.png'));
